...
## 歷史資料倉（讓 CMoney 只抓最新一期）

所有 CMoney 原始資料的完整歷史存在 `archive/` 底下的 17 個 CSV（全部約 4MB），
更新時 CMoney 只要抓最新 1~2 期，程式會以「期別」為鍵覆蓋進資料倉，網站再從資料倉拿完整歷史。
`FwdPE_周`、`加權Fwd.EPS`、`預估明年EPS成長` 這三張純公式表已改由 Python 重算，Excel 裡可以刪掉。

第一次啟用、CMoney 的 TOP N 要怎麼調、以及驗證結果，全部寫在 **`增量更新_操作說明.md`**。

`refresh_data.py` 看到 `archive/` 存在就會自動走這條路；把它移走就會退回原本直接讀 Excel 的模式。
`archive/` 沒有進 GitHub（CMoney 付費資料），請自行另外備份。
