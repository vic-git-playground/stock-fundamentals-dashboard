...
查詢一檔股票後，「手動調整目前股價」下面有「EPS 基準」四選一：

- **加權 EPS**（預設）：跟 Excel 一樣，依月份在今年／明年預估之間加權，越接近年底越偏明年
- **今年預估 EPS**：純粹用今年的機構預估
- **明年預估 EPS**：純粹用明年的機構預估
- **自行填寫 EPS**：自己輸入一個數字，整段歷史都用這個固定值當分母

切換後會影響三個地方：**PE Band**、**Fwd. PE 走勢**、**評價試算器**。因為換了分母等於整條 PE 序列
都變了，所以三年滾動的高／中／低 PE 倍數會在瀏覽器端整條重算，不是沿用匯出時算好的值——
你會看到 Band 的上下緣跟著移動，這是正常的。PB 圖不受影響（PB 的分母是每股淨值，跟 EPS 無關）。

選擇會記在瀏覽器裡（localStorage），下次打開沿用；自行填寫的數字是每檔股票分開記的。
可以跟「手動調整股價」併用，例如股價填 2000、EPS 填 60，Fwd. PE 就會是 33.3。
