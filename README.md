...
有些圖偶爾會出現極端值——例如某一期預估 EPS 接近 0，Fwd. PE 會衝到 800 倍，自動縮放之後其他年份
的走勢就被壓成貼著底部的一條線。八張圖都可以自己調 Y 軸：

- **在左右兩側的刻度上上下拖曳**：放大／縮小範圍。支點是你按住的那個刻度（不是圖的中心），
  所以想看下方那段就按住下方的刻度往上推，畫面會朝那裡收斂
- **在圖表區域內上下拖曳**：整段範圍上下平移，寬度不變
- **在圖上點兩下**：回到自動縮放

每張圖分開記，重新查詢股票或切換 EPS 基準時會回到自動縮放。這只是看圖的顯示範圍，不會動到資料。

## EPS 基準（PE 的分母要用哪個 EPS）

查詢一檔股票後，「手動調整目前股價」下面有「EPS 基準」三選一，決定**整條 EPS 序列**：

- **加權 EPS**（預設）：跟 Excel 一樣，依月份在今年／明年預估之間加權，越接近年底越偏明年
- **今年預估 EPS**：用今年的機構預估。旁邊可以填一個**轉換月** N，表示每年 N 月（含）以後改用
  明年預估、N 月之前用今年預估；留空就整年都用今年預估。例如填 6，則 6~12 月用明年、1~5 月用今年
  （這是「硬切換」，跟加權的漸進式不一樣）
- **明年預估 EPS**：用明年的機構預估

再下面有一個獨立的 **「自行填寫最新一期 EPS」**：只會取代**最新一期**的 EPS，歷史各期還是照上面選的
基準計算。用途跟手動股價一樣，是拿來試算「如果 EPS 變成這個數字，現在的 PE 與合理價會是多少」。

影響三個地方：**PE Band**、**Fwd. PE 走勢**、**評價試算器**。因為換基準等於整條 PE 序列的分母都變了，
三年滾動的高／中／低 PE 倍數會在瀏覽器端整條重算，不是沿用匯出時算好的值——你會看到 Band 的上下緣
跟著移動，這是正常的。PB 圖不受影響（PB 的分母是每股淨值，跟 EPS 無關）。

基準和轉換月是全域設定，自行填寫的 EPS 則是每檔股票分開記，都存在瀏覽器的 localStorage。
可以跟「手動調整股價」併用，例如股價填 2000、最新一期 EPS 填 60，Fwd. PE 就會是 33.3。
